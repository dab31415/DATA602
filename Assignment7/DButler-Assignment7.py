...
def exercise01():
    ...
    # ------ Place code below here \/ \/ \/ ------
    df_first_five_rows = pd.DataFrame(iris.data, columns = iris.feature_names).head(5)
...
def exercise02(new_observations):
    ...
    knn_m = KNN(n_neighbors=5)
    knn_m.fit(X,y)
    df5, target_names = exercise01()
    
    predictions = knn_m.predict(new_observations)
    iris_predictions = []
    for x in predictions:
        iris_predictions.append(target_names[x])
...
def exercise04():
    ...
    df = pd.DataFrame(list(zip(n_range,score_train,score_test)), columns = ['k','train','test'])

    fig = px.line(df, x = 'k', y = df.columns[1:], labels = {'k':'neighbors','variable':'accuracy series','value':'performance'}, title = 'kNN Testing')
    
    chart_studio.tools.set_credentials_file(username = Username, api_key = APIKey)
    plotly_overfit_underfit_curve_url = py.plot(fig, filename = 'Accuracy', auto_open = True)

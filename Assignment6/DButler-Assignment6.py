...
def exercise09():
    ...
    response = requests.get('https://api.coindesk.com/v1/bpi/historical/close.json?start=2017-09-01&end=20118-10-05', headers = header)
    res_j = response.json()
...
def exercise11(n):
    ...
    array_reshaped = np.reshape(array_1d, (n/3,3))
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.tile(exercise10(2),(n,n))
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(data=np.random.randint(n,size=n), index = np.arange(np.datetime64('2020-01-01'),np.datetime64('2020-01-01')+n))
...
def exercise15():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df = exercise08()[0].iloc[::5,:].loc[:,['street','zip']]

...
class PandasChain:
    ...
    def __init__(self, name): 
        ...
        self.__current_block = Block(seq_id = self.__seq_id, prev_hash = None) # Create a new Block 
    ...
    def __commit_block(self,block): 
        # Add code here
        ts = dt.datetime.now()
        merkle_hash = self.__current_block.get_simple_merkle_root()
        block_hash = get_hash(str(self.__prev_hash) + str(self.__id) + str(ts) + str(self.__seq_id) + str(np.random.randint(100)) + str(merkle_hash))
...
class Block:
    ...
    def add_transaction(self, s, r, v): 
        ts = dt.datetime.now()
        tx_hash = get_hash(str(ts) + str(s) + str(r) + str(v))
        ...
        self.__merkle_tx_hash = get_hash(self.__transactions['TxHash'].str.cat())
